...
var code = {
    register: function(memlink, creep, override=false, overridebase={}) {
    ...
    harvest: function(creep) {
        ...
        if(errlog == ERR_NOT_IN_RANGE) {
            creep.moveTo(sourceid, {reusePath: 20, visualizePathStyle: {stroke: '#ffffff'}});
        }
        return errlog
    },
    
    remove: function(memlink, creep, override=false, overridebase=0) {
        // If the creep has a registered source
        if(creep.memory.registeredsource !== 0 || creep.memory.registeredsource !== undefined) {
            if(override) {
                // If override is enabled, use overridebase to remove the source ID
                overridebase[memlink] = funcs.Lremove(overridebase[memlink],creep.memory.registeredsource)
                creep.memory.registeredsource = 0;
            } else {
                // If override is disabled, use Memory[memlink] to remove the source ID
                Memory[memlink] = funcs.Lremove(Memory[memlink],creep.memory.registeredsource)
                creep.memory.registeredsource = 0;
            }
        }
    }
